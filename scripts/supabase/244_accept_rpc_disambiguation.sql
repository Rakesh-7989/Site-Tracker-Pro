-- 244_accept_rpc_disambiguation.sql
-- The RETURNS TABLE OUT-params (project_id/org_id/project_name) shadowed
-- unqualified column references inside the body ("column reference
-- project_id is ambiguous"). Qualify every predicate against its alias.
...
do $$
begin
  raise notice '244 accept RPC predicates fully qualified';
end $$;
