...
Deno.serve(async (req) => {
  ...
  if (Deno.env.get("MH_RERA_SCRAPER_ENABLED") !== "true") {
    return json({ error: "scraper-disabled", hint: "Set MH_RERA_SCRAPER_ENABLED=true after wiring credentials." }, 503, ch);
  }
  ...
  if (!period || !PERIODS.includes(period)) return json({ error: "period-required", hint: PERIODS.join("|") }, 400, ch);
  ...
  return json({ ok: true, ack_no: ackNo, state: "MH", period }, 200, ch);
});
