...
describe("mbSelectionTotal", () => {
  it("sums amounts, ignoring missing", () => {
    expect(mbSelectionTotal([{ amount: 100 }, { amount: null }, { amount: 25 }] as any)).toBe(125);
  });
});

describe("listUnlinkedMb (ST-019)", () => {
  it("maps rows to camelCase", async () => {
    const client: any = { from: () => ({ select: () => ({ eq: () => ({ is: () => ({ order: () => ({ limit: async () => ({ data: [{ id: "1", mb_no: "MB-1", page_no: 3, description: "RCC slab", unit: "cum", qty: 10, rate: 4500, amount: 45000, status: "verified" }], error: null }) }) }) }) }) }) };
...
describe("listMbForRa (ST-019)", () => {
  it("returns linked rows", async () => {
    const client: any = { from: () => ({ select: () => ({ eq: () => ({ order: async () => ({ data: [{ id: "1", mb_no: "MB-9", description: "Brick", qty: 2, amount: 200, status: "verified" }], error: null }) }) }) }) };
...
describe("link / unlink", () => {
  it("links a batch of MB rows to an RA bill", async () => {
    const client: any = { from: () => ({ update: () => ({ in: async () => ({ error: null }) }) }) };
    const r = await linkMbToRa(client, ["a", "b"], "ra1");
    expect(r.ok).toBe(true);
  });
  it("unlinks a MB row", async () => {
    const client: any = { from: () => ({ update: () => ({ eq: async () => ({ error: null }) }) }) };
