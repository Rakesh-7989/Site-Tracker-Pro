...
describe("getOrgPurchaseOrders", () => {
  it("maps fields + coerces status; surfaces error", async () => {
    const r = await getOrgPurchaseOrders(rpcClient({ data: [
      { id: "1", po_no: "PO-1", project_id: "p1", project_name: "Tower A", vendor_name: "ACC", items: "Cement", amount: 80000, status: "approved", created_date: "2026-06-01", delivery_date: "2026-06-15" },
...
describe("poTotals", () => {
  const mk = (amount: number, status: CrossPO["status"]): CrossPO => ({ id: "x", poNo: "", projectId: "p", projectName: "", vendorName: null, items: null, amount, status, createdDate: null, deliveryDate: null });
