...
function Stat({ label, value, accent }: { label: string; value: number; accent?: boolean }): JSX.Element {
  return (
    <Card className="p-4">
      <div className={`text-3xl font-display font-bold ${accent ? "text-accent" : "text-fg-primary"}`}>{value}</div>
    ...
    <div className="max-w-5xl mx-auto space-y-6">
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <div>
          ...
          <div className="text-sm text-fg-secondary">Cross-tenant overview</div>
        </div>
      ...
      {loading ? <div className="grid place-items-center py-12"><Spinner size={24} /></div> : (
        <>
          ...
          {/* Plan mix */}
          <div>
            <h2 className="text-xs font-semibold tracking-[0.16em] uppercase text-fg-tertiary mb-2">Plan mix</h2>
            <Card className="p-4 flex flex-wrap gap-4">
              {PLAN_ORDER.map(p => (
                <div key={p} className="text-center min-w-[64px]">
                  <div className="text-2xl font-bold text-fg-primary">{s?.plans?.[p] ?? 0}</div>
                  <div className="text-[11px] text-fg-secondary">{PLAN_LABEL[p] ?? p}</div>
                </div>
              ))}
              <div className="text-center min-w-[64px] border-l border-default pl-4">
                <div className="text-2xl font-bold text-fg-primary">{s?.approvedSignups ?? 0}</div>
                <div className="text-[11px] text-fg-secondary">Approved</div>
              </div>
              <div className="text-center min-w-[64px]">
                <div className="text-2xl font-bold text-fg-primary">{s?.staffCount ?? 0}</div>
                <div className="text-[11px] text-fg-secondary">Staff</div>
              </div>
            </Card>
          </div>
                ...
                <Link key={l.to} to={l.to}>
                  <Card className="p-4 hover:border-accent transition cursor-pointer h-full relative">
                    {l.to === "/admin/signups" && (s?.pendingSignups ?? 0) > 0 && (
                      <span className="absolute top-3 right-3 text-[10px] font-bold bg-accent text-white rounded-full px-1.5 py-0.5">{s?.pendingSignups}</span>
                    )}
