  ...
  return (
    <div className="p-4 md:p-10 max-w-4xl">
      <div className="flex items-end justify-between mb-8 pb-3 flex-wrap gap-3 border-b border-default">
        <div>
          <div className="text-[10px] font-bold tracking-[0.28em] uppercase text-accent-2 mb-2">— Workflow</div>
      ...
      <div className="bg-panel rounded-2xl overflow-hidden shadow-editorial border-default">
        {myDelegations.length === 0 ? <div className="p-12 text-center text-fg-secondary"><Icon name="users" size={32} className="mx-auto mb-2 opacity-30" /><p className="text-sm">No delegations yet.</p></div> : myDelegations.map(d => {
          ...
          return (<div key={d.id} className="p-4 flex items-center gap-3 flex-wrap border-b border-default">
            <span className={`text-[10px] font-bold tracking-wider uppercase px-2 py-0.5 rounded-full ${st === "active" ? "bg-success-tint text-success" : st === "scheduled" ? "bg-warning-tint text-warning" : st === "expired" ? "bg-secondary text-fg-primary" : "bg-error-tint text-error"}`}>{st}</span>
            <div className="flex-1 min-w-0"><div className="text-sm font-semibold text-fg-primary">→ {d.toUserName} <span className="text-[10px] font-mono text-fg-secondary">({d.scope})</span></div><div className="text-[11px] text-fg-secondary">{fmtDate(d.start)} → {fmtDate(d.end)}{d.reason && ` · ${d.reason}`}</div></div>
            {d.active !== false && st !== "expired" && <button onClick={() => revoke(d.id)} className="text-[11px] font-bold text-fg-secondary hover:text-error">Revoke</button>}
          </div>);
        })}
      </div>
    </div>
