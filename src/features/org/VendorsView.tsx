...
function Inner({ orgId, canManage }: { orgId: string; canManage: boolean }): JSX.Element {
  ...
  const columns = [
    { key: "name", header: "Vendor", render: (v: Vendor) => (
      <div>
        <div className="flex items-center gap-2"><span className="font-semibold text-fg-primary truncate">{v.name}</span>{v.category && <Badge tone="neutral">{v.category}</Badge>}</div>
        <div className="text-[11px] text-fg-tertiary">{[v.phone, v.gst && `GST ${v.gst}`].filter(Boolean).join(" \u00b7 ") || "\u2014"}</div>
      </div>
    )},
    { key: "rating", header: "Rating", render: (v: Vendor) => (
      canManage ? (
        <select className="text-xs bg-transparent text-warning" value={v.rating ?? 0} onChange={e => { const n = Number(e.target.value); void run(`r-${v.id}`, c => setVendorRating(c, v.id, n), { apply: () => setRows(prev => prev.map(x => x.id === v.id ? { ...x, rating: n } : x)), rollback: () => setRows(prev => prev.map(x => x.id === v.id ? { ...x, rating: v.rating } : x)) }); }}>
          <option value={0}>Rate\u2026</option>{[1, 2, 3, 4, 5].map(n => <option key={n} value={n}>{n} \u2605</option>)}
        </select>
      ) : (
        <span className="text-warning text-sm">{v.rating == null ? <span className="text-fg-tertiary">\u2014</span> : "\u2605".repeat(Math.round(v.rating)) + "\u2606".repeat(Math.max(0, 5 - Math.round(v.rating)))}</span>
      )
    )},
    ...(canManage ? [{ key: "actions", header: "", render: (v: Vendor) => (
      <Button size="sm" variant="ghost" onClick={() => void run(`d-${v.id}`, c => deleteVendor(c, v.id), { apply: () => setRows(prev => prev.filter(x => x.id !== v.id)), rollback: () => setRows(prev => [...prev, v]) })}><Icon name="trash" size={14} className="text-error" /></Button>
    )}] : []),
  ];
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-4 p-4 md:p-6">
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <h1 className="font-display text-xl md:text-2xl font-bold text-fg-primary">Vendors</h1>
        <span className="text-sm text-fg-secondary">{rows.length} total</span>
      </div>
      <p className="text-sm text-fg-secondary -mt-2">Material suppliers &amp; subcontractors shared across your projects.</p>
