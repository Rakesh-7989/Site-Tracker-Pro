...
const fmtCur = (n: any) => { if (n == null || n === "") return "â€”"; const num = Number(n); if (!Number.isFinite(num)) return "â€”"; return "â‚¹" + num.toLocaleString("en-IN"); };
...
function Inner({ orgId }: { orgId: string }): JSX.Element {
  ...
  if (dataLoading) return <div className="grid place-items-center p-12"><Spinner size={24} /><span className="mt-3 text-sm text-ink-500">Loading project dataâ€¦</span></div>;
  ...
  return (
    <div className="p-4 md:p-10 max-w-5xl">
      <div className="flex items-end justify-between mb-8 pb-3 flex-wrap gap-3" style={{ borderBottom: "1px solid var(--st-line)" }}>
        <div>
          <div className="text-[10px] font-bold tracking-[0.28em] uppercase text-amber-700 mb-2">â€” AI advisor</div>
      ...
      <div className="mb-5 flex items-center gap-3 flex-wrap">
            <button onClick={runForecast} disabled={busy} className="inline-flex items-center gap-2 px-5 py-2.5 bg-gradient-gold text-white font-bold rounded-xl text-sm tracking-wide hover:shadow-editorial-deep disabled:opacity-60"><Icon name="zap" size={14} />{busy ? "Forecastingâ€¦" : cached ? "Re-forecast" : "Run forecast"}</button>
          ...
          {cached ? (<>
            ...
            {cached.narrative && <div className="bg-white rounded-2xl p-5 mb-5 shadow-editorial" style={{ border: "1px solid var(--st-line)" }}><div className="text-[10px] font-bold tracking-[0.24em] uppercase text-amber-700 mb-2">â€” Advisor narrative ({cached.mode === "llm" ? "LLM-enriched" : "deterministic"})</div><p className="text-ink-800 text-sm leading-relaxed">{cached.narrative}</p></div>}
            {cached.over_consumed_materials?.length > 0 && <div className="bg-white rounded-2xl p-5 mb-5 shadow-editorial" style={{ border: "1px solid var(--st-line)" }}><div className="text-[10px] font-bold tracking-[0.24em] uppercase text-amber-700 mb-3">â€” Materials trending over plan</div><div className="space-y-2">{cached.over_consumed_materials.map((m: any) => (<div key={m.name} className="flex items-center justify-between text-sm"><span className="font-semibold text-ink-900 capitalize">{m.name}</span><span className="text-red-700 font-mono">{m.planned} â†’ {m.consumed} (<strong>+{m.over_pct}%</strong>)</span></div>))}</div></div>}
            <div className="text-[11px] text-ink-500 text-center">Schedule slip: <strong>{cached.schedule_slip_days} days</strong> Â· Confidence: <strong>{cached.confidence}</strong></div>
