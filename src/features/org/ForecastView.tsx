      ...
      <div className="mb-5 flex items-center gap-3 flex-wrap">
          ...
          {cached ? (<>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-5">
              <div className="bg-white rounded-2xl p-4 shadow-editorial border-st-line"><div className="text-[10px] font-bold uppercase tracking-[0.18em] text-ink-500 mb-1">Budget</div><div className="font-display text-xl font-bold text-ink-900">{fmtCur(cached.budget)}</div></div>
              <div className="bg-white rounded-2xl p-4 shadow-editorial border-st-line"><div className="text-[10px] font-bold uppercase tracking-[0.18em] text-ink-500 mb-1">Billed so far</div><div className="font-display text-xl font-bold text-ink-900">{fmtCur(cached.billed_so_far)}</div></div>
              <div className="bg-white rounded-2xl p-4 shadow-editorial border-st-line"><div className="text-[10px] font-bold uppercase tracking-[0.18em] text-ink-500 mb-1">Projected total</div><div className="font-display text-xl font-bold text-ink-900">{fmtCur(cached.projected_total)}</div></div>
              <div className={`rounded-2xl p-4 shadow-editorial border-st-line ${cached.overrun_amount > 0 ? "bg-red-50" : "bg-emerald-50"}`}><div className={`text-[10px] font-bold uppercase tracking-[0.18em] mb-1 ${cached.overrun_amount > 0 ? "text-red-700" : "text-emerald-700"}`}>Likely overrun</div><div className={`font-display text-xl font-bold ${cached.overrun_amount > 0 ? "text-red-700" : "text-emerald-700"}`}>{cached.overrun_amount > 0 ? `+${fmtCur(cached.overrun_amount)} (${cached.overrun_pct}%)` : "On track"}</div></div>
            </div>
            {cached.narrative && <div className="bg-white rounded-2xl p-5 mb-5 shadow-editorial border-st-line"><div className="text-[10px] font-bold tracking-[0.24em] uppercase text-amber-700 mb-2">— Advisor narrative ({cached.mode === "llm" ? "LLM-enriched" : "deterministic"})</div><p className="text-ink-800 text-sm leading-relaxed">{cached.narrative}</p></div>}
            {cached.over_consumed_materials?.length > 0 && <div className="bg-white rounded-2xl p-5 mb-5 shadow-editorial border-st-line"><div className="text-[10px] font-bold tracking-[0.24em] uppercase text-amber-700 mb-3">— Materials trending over plan</div><div className="space-y-2">{cached.over_consumed_materials.map((m: any) => (<div key={m.name} className="flex items-center justify-between text-sm"><span className="font-semibold text-ink-900 capitalize">{m.name}</span><span className="text-red-700 font-mono">{m.planned} → {m.consumed} (<strong>+{m.over_pct}%</strong>)</span></div>))}</div></div>}
