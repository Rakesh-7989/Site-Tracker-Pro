  ...
  return (
    <div className="p-4 md:p-10 max-w-6xl">
      ...
      <div className="bg-panel rounded-2xl p-5 mb-5 grid sm:grid-cols-5 gap-3 shadow-editorial border-default">
            <div><label className="text-[10px] font-bold tracking-[0.24em] uppercase text-fg-secondary mb-1.5 block">Commodity</label><Select value={commodity} onChange={e => setCommodity(e.target.value)} options={Object.entries(COMMODITIES).map(([k, v]) => ({ value: k, label: v.label }))} /></div>
            <div><label className="text-[10px] font-bold tracking-[0.24em] uppercase text-fg-secondary mb-1.5 block">Grade</label><Select value={grade} onChange={e => setGrade(e.target.value)} options={grades.map((g: string) => ({ value: g, label: g }))} /></div>
            <div><label className="text-[10px] font-bold tracking-[0.24em] uppercase text-fg-secondary mb-1.5 block">Qty ({COMMODITIES[commodity as keyof typeof COMMODITIES]?.unit})</label><input type="number" min="1" value={qty} onChange={e => setQty(Number(e.target.value))} className="w-full p-2.5 border border-default rounded-xl text-sm outline-none focus:border-accent" /></div>
