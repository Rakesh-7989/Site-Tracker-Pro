...
export function ProfileView(): JSX.Element {
  ...
  const lockedCls = "w-full px-3.5 py-2.5 border border-default rounded-lg text-sm bg-panel text-fg-secondary cursor-not-allowed";
    ...
    <div className="max-w-2xl mx-auto">
      <div className="flex items-center justify-between mb-5">
        <div className="flex items-center gap-2">
          <Link to="/dashboard" title={t("profile.back")} className="w-8 h-8 rounded-lg grid place-items-center text-fg-secondary hover:bg-secondary transition">
      ...
      {error && <div className="mb-3 rounded-lg bg-error-tint border border-error p-3 text-[13px] text-error flex items-start gap-2"><Icon name="alert" size={15} className="text-error mt-0.5" /> {error}</div>}
      {saved && (
      ...
      <Card className="p-5">
        <div className="flex items-center gap-3 mb-4">
          <div className="w-12 h-12 rounded-full bg-ink text-white grid place-items-center font-bold">{(name || session.user.name || "U").slice(0, 2).toUpperCase()}</div>
          <div>
