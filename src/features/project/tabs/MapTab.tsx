...
export function MapTab({ project }: { project: ProjectDetail }): JSX.Element {
  ...
  return (
    <div className="space-y-4">
      <h2 className="font-display text-lg font-bold text-fg-primary">Site location</h2>
      <Card className="p-6 text-center">
        <div className="w-14 h-14 rounded-2xl bg-accent-tint text-accent grid place-items-center mx-auto mb-3"><Icon name="map" size={26} /></div>
        <div className="font-semibold text-fg-primary">{project.name}</div>
        <div className="text-sm text-fg-secondary mt-0.5">{project.location || "No location set for this project."}</div>
        {project.location && (
