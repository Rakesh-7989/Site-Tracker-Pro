    ...
    <div className="space-y-4">
      ...
      {balance.length > 0 && (
        <Card className="p-3">
          ...
          <div className="flex flex-wrap gap-2">{balance.map(([m, v]) => <span key={m} className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[11px] bg-secondary text-fg-primary">{m}: <b>{v}</b></span>)}</div>
        </Card>
      )}
      {canEdit && (
