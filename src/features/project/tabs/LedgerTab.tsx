    ...
    <div className="space-y-4">
      ...
      {balance.length > 0 && (
        <Card className="p-3">
          ...
          <div className="flex flex-wrap gap-2">{balance.map(([m, v]) => <span key={m} className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[11px] bg-bg-secondary text-fg-primary">{m}: <b>{v}</b></span>)}</div>
        </Card>
      )}
      {canEdit && (
        ...
        : <div className="space-y-2">{rows.map(r => (
            <Card key={r.id} className="p-3 flex items-center justify-between gap-3">
              <div className="min-w-0"><div className="text-sm font-semibold text-fg-primary truncate flex items-center gap-2"><Badge tone={dirTone(r.direction)}>{r.direction}</Badge>{r.material} · {r.qty}{r.unit ? ` ${r.unit}` : ""}</div>
                <div className="text-[11px] text-fg-tertiary">{r.txnDate}{r.refNo ? ` · ${r.refNo}` : ""}</div></div>
