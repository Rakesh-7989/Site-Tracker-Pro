// SiteTrack Pro â€” project Labour register tab (v3 port, DB-wired). Statutory
    ...
    <div className="space-y-4">
      ...
      {canEdit && (
        <Card className="p-3 flex gap-2 flex-wrap items-end">
          ...
          <div><span className="text-[11px] font-semibold uppercase tracking-wider text-ink-400">Daily wage â‚¹</span><Input className="mt-1 w-24" type="number" value={wage} onChange={e => setWage(e.target.value)} /></div>
      ...
      {loading ? <div className="grid place-items-center py-10"><Spinner size={22} /></div>
        ...
        : <div className="space-y-2">{rows.map(r => (
            <Card key={r.id} className="p-3 flex items-center justify-between gap-3">
              <div className="min-w-0"><div className="text-sm font-semibold text-ink-800 truncate">{r.name}{r.trade ? <span className="text-ink-400 font-normal"> Â· {r.trade}</span> : null}</div>
