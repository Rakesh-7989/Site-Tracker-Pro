  ...
  return (
    <Card padding="md" className="space-y-3">
      <div className="flex items-center justify-between flex-wrap gap-2">
        <h2 className="font-display text-base font-bold text-fg-primary">{t("partner.sharedTitle")}</h2>
        <div className="flex items-center gap-2">
          <Input
            fit
            value={code}
            onChange={e => setCode(e.target.value)}
            placeholder={t("partner.pasteCode")}
            aria-label={t("partner.codeAriaLabel")}
      ...
      {acceptedProjectId && (
        <Alert variant="success">
          {t("partner.inviteAccepted")} <Link to={`/projects/${acceptedProjectId}`} className="text-accent underline">{t("partner.openProject")}</Link>.
        </Alert>
      )}
