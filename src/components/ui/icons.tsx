// SiteTrack Pro — typed icon set (Phase 4 design-system migration).
//
// Single source for every Lucide-style SVG the app uses. The legacy
// src/components/ui.jsx `Ic` component is mirrored here with a strict
// IconName union so callers get autocomplete + compile-time checks
// instead of silently rendering null on a typo.
//
// Stroke icons share one <svg> wrapper; the per-icon entry is just the
// inner path(s). camelCase names match the legacy set so the eventual
// App.jsx migration is a mechanical find-replace.

import type { ReactNode } from "react";

/** Inner path geometry per icon. */
const PATHS = {
  ...
  eyeOff: <><path d="M9.88 9.88a3 3 0 1 0 4.24 4.24" /><path d="M10.73 5.08A10.43 10.43 0 0 1 12 5c7 0 11 7 11 7a13.16 13.16 0 0 1-1.67 2.68" /><path d="M6.61 6.61A13.526 13.526 0 0 0 1 12s4 7 11 7a9.74 9.74 0 0 0 5.39-1.61" /><line x1="2" x2="22" y1="2" y2="22" /></>,
  ...
  doc: <><path d="M14.5 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V7.5L14.5 2z" /><polyline points="14 2 14 8 20 8" /><line x1="16" x2="8" y1="13" y2="13" /><line x1="16" x2="8" y1="17" y2="17" /><line x1="10" x2="8" y1="9" y2="9" /></>,
  ...
  phone: <path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07A19.5 19.5 0 0 1 4.69 13a19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 3.44 2h3a2 2 0 0 1 2 1.72c.127.96.361 1.903.7 2.81a2 2 0 0 1-.45 2.11L8.09 9.91a16 16 0 0 0 6 6l1.27-1.27a2 2 0 0 1 2.11-.45c.907.339 1.85.573 2.81.7A2 2 0 0 1 22 16.92z" />,
...
export interface IconProps {
  ...
  size?: number;
  className?: string;
  /** Extra stroke width override (defaults vary subtly by icon family). */
  strokeWidth?: number;
}

export function Icon({ name, size = 18, className = "", strokeWidth = 2 }: IconProps): JSX.Element {
  const inner: ReactNode = PATHS[name];
  return (
    <svg
      width={size}
      height={size}
...
/** Runtime check — useful for tests + dynamic icon names. */
export function isIconName(value: unknown): value is IconName {
  return typeof value === "string" && Object.prototype.hasOwnProperty.call(PATHS, value);
}

/** All icon names (for the design-system gallery + tests). */
