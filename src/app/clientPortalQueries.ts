...
export interface ProjectBrief { id: string; name: string; location: string | null; status: string; progress: number; client_email: string | null; }
...
export async function listClientProjects(client: any, email: string): Promise<PResult<ProjectBrief[]>> {
  try {
    ...
    return { ok: true, data: (data ?? []).map((r: any) => ({ id: r.id, name: r.name, location: r.location, status: r.status, progress: r.progress ?? 0, client_email: r.client_email })) };
