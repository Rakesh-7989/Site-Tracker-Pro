...
type DynBuilder = {
  update(p: Record<string, unknown>): { eq(c: string, v: unknown): Promise<{ error: unknown }> };
  delete(): { eq(c: string, v: unknown): Promise<{ error: unknown }> };
};
const dynFrom = (client: TypedSupabaseClient, table: string): DynBuilder => (client as unknown as { from: (t: string) => DynBuilder }).from(table);
async function upd(client: TypedSupabaseClient, table: string, id: string, patch: Record<string, unknown>): Promise<Result<{ ok: true }>> {
  try { const { error } = await dynFrom(client, table).update(patch).eq("id", id); if (error) return dbe(error); return ok({ ok: true }); } catch (e) { return er(e); }
}
async function del(client: TypedSupabaseClient, table: string, id: string): Promise<Result<{ ok: true }>> {
  try { const { error } = await dynFrom(client, table).delete().eq("id", id); if (error) return dbe(error); return ok({ ok: true }); } catch (e) { return er(e); }
}
...
export async function listDrawings(client: TypedSupabaseClient, projectId: string): Promise<Result<Drawing[]>> {
  try {
    ...
    return ok(((data ?? []) as Array<Record<string, unknown>>).map((r: Record<string, unknown>) => ({
      id: String(r.id),
...
export async function createDrawing(client: TypedSupabaseClient, input: { projectId: string; title: string; type: string; revision?: string; releasedBy: string }): Promise<Result<{ id: string }>> {
  try {
...
export const setDrawingStatus = (client: TypedSupabaseClient, id: string, status: DrawingStatus) => upd(client, "drawings", id, { status });
/** Persist a per-drawing design-workflow stage (Phase E Opt3, migration 166). */
export const setDrawingStage = (client: TypedSupabaseClient, id: string, designStage: string) => upd(client, "drawings", id, { design_stage: designStage });
/** Persist the preferred raster preview file path (migration 150) for the D2 diff overlay. */
export const setDrawingPreviewUrl = (client: TypedSupabaseClient, id: string, previewUrl: string | null) => upd(client, "drawings", id, { preview_url: previewUrl });
export const deleteDrawing = (client: TypedSupabaseClient, id: string) => del(client, "drawings", id);
...
export async function listRfis(client: TypedSupabaseClient, projectId: string): Promise<Result<Rfi[]>> {
  try {
    ...
    if (error) return dbe(error);
    return ok(((data ?? []) as Array<Record<string, unknown>>).map((r: Record<string, unknown>) => ({ id: String(r.id), no: String(r.no ?? ""), subject: String(r.subject ?? ""), question: String(r.question ?? ""), category: r.category == null ? null : String(r.category), status: asRfi(r.status), response: r.response == null ? null : String(r.response), askedAt: r.asked_at == null ? null : String(r.asked_at) })));
  } catch (e) { return er(e); }
}
export async function createRfi(client: TypedSupabaseClient, input: { projectId: string; no: string; subject: string; question: string; category?: string; askedBy: string }): Promise<Result<{ id: string }>> {
  try {
...
export const respondRfi = (client: TypedSupabaseClient, id: string, response: string) => upd(client, "rfi", id, { response, status: "answered" });
export const setRfiStatus = (client: TypedSupabaseClient, id: string, status: RfiStatus) => upd(client, "rfi", id, { status });
export const deleteRfi = (client: TypedSupabaseClient, id: string) => del(client, "rfi", id);
...
export async function listChangeOrders(client: TypedSupabaseClient, projectId: string): Promise<Result<ChangeOrder[]>> {
  try {
    ...
    if (error) return dbe(error);
    return ok(((data ?? []) as Array<Record<string, unknown>>).map((r: Record<string, unknown>) => ({ id: String(r.id), no: String(r.no ?? ""), description: String(r.description ?? ""), costImpact: r.cost_impact == null ? null : Number(r.cost_impact), scheduleImpact: r.schedule_impact == null ? null : Number(r.schedule_impact), reason: r.reason == null ? null : String(r.reason), status: asCo(r.status) })));
  } catch (e) { return er(e); }
}
export async function createChangeOrder(client: TypedSupabaseClient, input: { projectId: string; no: string; description: string; costImpact?: number; scheduleImpact?: number; reason?: string; raisedBy: string }): Promise<Result<{ id: string }>> {
  try {
...
export const setCoStatus = (client: TypedSupabaseClient, id: string, status: CoStatus) => upd(client, "change_orders", id, { status });
export const deleteChangeOrder = (client: TypedSupabaseClient, id: string) => del(client, "change_orders", id);
...
export async function listEstimates(client: TypedSupabaseClient, projectId: string): Promise<Result<Estimate[]>> {
  try {
...
export async function createEstimate(client: TypedSupabaseClient, input: { projectId: string; name: string; totalAmount: number; version?: number; payload?: Record<string, unknown>; createdBy: string }): Promise<Result<{ id: string }>> {
  try {
    const { data, error } = await client.from("estimate").insert({ project_id: input.projectId, name: input.name, total_amount: input.totalAmount, version: input.version ?? 1, payload: input.payload ?? {}, created_by: input.createdBy } as never).select("id").single();
    if (error) return dbe(error); return ok({ id: String(data.id) });
  } catch (e) { return er(e); }
}
export const setEstimateStatus = (client: TypedSupabaseClient, id: string, status: EstimateStatus) => upd(client, "estimate", id, { status });
export const deleteEstimate = (client: TypedSupabaseClient, id: string) => del(client, "estimate", id);
