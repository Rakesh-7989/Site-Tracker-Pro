...
export function validateGstin(g: string): GstinResult {
...
export function validateHsn(h: string): HsnResult {
...
export function buildEInvoicePayload({
  ...
  if (!SUPPLY_TYPES[supplyType as keyof typeof SUPPLY_TYPES]) return { ok: false, reason: "invalid-supply-type" };
  ...
  const totalValue = validatedItems.reduce((s, it) => s + (it.TotItemVal as number || 0), 0) as number;
  const totalCgst = validatedItems.reduce((s, it) => s + (it.CgstAmt as number || 0), 0) as number;
  const totalSgst = validatedItems.reduce((s, it) => s + (it.SgstAmt as number || 0), 0) as number;
...
export const gstnAdapter = {
  async submit(payload: Record<string, unknown>, { endpoint, token }: { endpoint?: string; token?: string } = {}) {
