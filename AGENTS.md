...
**State**: `main` = `7379c5c` (pushed, origin synced — **not** on prod; prod ships via founder-approved main→prod PR); migrations **261** (`261_lifecycle_emails.sql`, immutable/applied) + **262** (`262_lifecycle_emails_recipient_nullable.sql`, applied cleanly) + the `supabase/functions/lifecycle-emails-cron/` EF were committed (`6daf1b5`), docs (`AGENTS.md` + `§2.4`) committed. 15 benign pre-existing checksum drifts (236 passed · 15 failed · 235 skipped) recur — ignore. Ops notes: the CLI's `supabase db execute` subcommand does NOT exist (use `db query` or MCP SQL); EF bearer = `notify_config.promoter_digest_cron_secret`; the next due event (day7 nudge, 2026-09-13) will fire on the existing cron job 356 @ 02:20 UTC.

**Post-ship verification (2026-09-10)**: queried Resend API `GET /emails/{id}` for both msgids → **`last_event=delivered`** for both `2fb8828c-…` (subj "Welcome to SiteTrack Pro - your 14-day trial is live") and `dd63ac1f-…` (subj "Need a hand setting up your first project?"), to `gangineniramakrishna766@gmail.com`. System-side confirmation complete; **all 7 templates** (day1/3/7/10/14/21/weekly) are implemented in the EF and fire automatically as orgs age — §2.4 has **0 remaining build work**.

**Remaining (founder/user steps only)**: visually confirm the two emails in Gmail (both already `delivered` per Resend); then the standing backlog (DXF visual test in Drawings, `*.sitetrackpro.in` wildcard CNAME at the registrar, TrackingCAA record, WhatsApp/Twilio/push/AI provider keys, mobile `.aab` via Android Studio). Lifecycle feature reaches prod via the next main→prod PR.
...
- **`scripts/ci/smoke.mjs`** — added `FfeRollupView` to the app-source scan + `FfeRollupView`/`ffeOrgRollup` markers (235 checks).
...
- **`scripts/ci/smoke.mjs`** — added `DownloadAuditView`, `downloadAuditQueries`, `logDownloadEvent` to the app-source scan (237 checks).
...
- **`scripts/ci/smoke.mjs`** — added `MonthlyStatementView`, `monthlyStatementTotals` markers (239 checks).
...
### Notes / Follow-ups
- Module ownership per tab documented in `docs/architecture/MODULES.md` §3 table (three-place consistency rule: migration 155 CHECK ↔ registry.ts ↔ i18n).
...
- `VoiceConfidenceBar.tsx` was dead code (never imported) — **removed 2026-08-07** (see below).
...
**Fixes (working tree)**: mig 257 plan_payments + payments.method gateway values; prices aligned to DB-93 truth (plans.ts + PLAN_META + Pricing/Product/Home + tests); new EF cashfree-plan-link (orgadmin JWT, upgrade/renew-only, DB price + GST, per-org 5/hr throttle, plan_payments pending); cashfree-webhook plan_upgrade settle+activate (plan + subscriptions + billing_history, idempotent) + subscription-ACTIVE plan flip; razorpay-webhook settlement rows + status paid on full (partial keeps status); billing pay* keys en/te/hi.

**Frontend (verified 2026-09-11, working tree)**: `src/app/queries/planPaymentQueries.ts` (new — `mintPlanPaymentLink` mirroring upgradeQueries error-unwrap); `OrgBillingView.tsx` — new `PayUpgradeCard` (basic/pro/business upgrade targets only vs `ORDER`, BillingPeriod Select Monthly/Annual, GST-incl. preview via `formatINR(gstInclusive(raw))`, enterprise→ticket hint, mint → `window.open(linkUrl)` + `?paid=1`); OrgBillingInner gains `useSearchParams` + `verifying` state + a recursive [6s,6s,8s] reload timer that clears `?paid=1` after 3 cycles, with an info `Alert` `billing.payVerifying` during verification. smoke.mjs: `planPaymentQueries.ts` added to app-source scan + 4 markers (`mintPlanPaymentLink`/`PayUpgradeCard`/`PAY_PLAN_TARGETS`/`payVerifying`) → **477 checks** (was 473; the older "476" record was aspirational — the markers were never committed until now).
...
### Session 2026-09-18 — Razorpay self-serve plan links shipped LIVE (✅ complete, PR #70 → squash `0afa2d8`)
- **Context**: paid-plans billing shipped in PR #67 (squash `ecb6c5e`) called the **Cashfree** `cashfree-plan-link` EF. This session replaced it with the Razorpay path end-to-end: new EF, webhook settlement, frontend caller, CI smoke markers.
- **Code (commit `bbd29a7`)**:
  - **`supabase/functions/razorpay-plan-link/index.ts`** (new, 180 lines): POST `{org_id, plan, period}` → JWT `authenticate` + org-scope validation → tier downgrade check → price from `plans` table → creates a hosted link via Razorpay Payment Links API → inserts `plan_payments` row (status `pending`, `link_id`, `amount_paise`, notes `{type:"plan_upgrade"}`) → returns `link_url`. Error codes: `razorpay-not-configured` (503), `downgrade-manual`, `no-price`, `org-not-found`, `rate-limited`, `stash-failed`, `link-failed`, `razorpay-fetch-failed`.
  - **`supabase/functions/razorpay-webhook/index.ts`** (255 lines added): `handlePlanPayment` settlement — fired when `notes.sitetrack_plan_payment_id` present **or** notes-less `payment.*` events whose `link_id` matches `plan_payments` (fallback). Settles row → `organizations.plan`/`billing_period` update (service_role bypasses 254 guard) → `subscriptions.upsert({provider:"razorpay"})` → `billing_history` insert (amount/gst 1.18 split) → `record_audit_v2` → admin confirmation email via Resend. Idempotent: settled rows ack without re-activating; late PAID on terminal rows ignored.
  - **`src/app/queries/planPaymentQueries.ts`**: `mintPlanPaymentLink` now invokes `razorpay-plan-link` (was `cashfree-plan-link`).
  - **`scripts/ci/smoke.mjs`**: marker `cashfree-plan-link` → `razorpay-plan-link`.
- **Gates (local)**: tsc clean, eslint clean (2 EF files are Deno — outside lint scope by design), build clean, smoke 477/477, vitest 3055-equivalent (241 files; single `uiCharts` pool timeout re-ran green in isolation).
- **Migration 257** (`plan_payments` table + `link_id` col) was already LIVE (applied in the grey zone 1078996). Verified live via read-only probe: table/columns present, ledger shows mig257 applied (255 passed / 1 benign pre-existing failure).
- **Live deploy (via `deploy-edge-functions.mjs` + direct CLI)**:
  - `razorpay-plan-link` **v1** (14:39:54) — JWT verify ON (gateway rejects unauthenticated: GET/POST → **401**).
  - `razorpay-webhook` **v12** (14:40:10) `--no-verify-jwt` — GET → **405** (POST-only), signature-less POST → **401** (EF-level reject).
  - Razorpay secrets confirmed live via `supabase secrets list`: `RAZORPAY_KEY_ID`, `RAZORPAY_KEY_SECRET`, `RAZORPAY_WEBHOOK_SECRET`.
- **PR lifecycle**: commit `bbd29a7` → push → PR #70 → initially **CONFLICTING** (main lagged prod; `AGENTS.md` docs conflict — merge-base `7d825e3` #68, prod already at `2a6748f` #69) → merge-back `10c409d` (`git checkout --ours AGENTS.md`, keeping main's superset incl. web-push + backlog sections) → **MERGEABLE/ALL checks green** (test ×2, e2e-mock ×2, coverage ×2, Supabase Preview) → squash **`0afa2d8`** onto prod. Origin/main == origin/prod **identical trees** (`5413ea94…`).
- **Vercel prod alias verification (deploy mechanism re-confirmed 4th time)**: the squash `0afa2d8` gets a **Preview** deployment only; production alias follows **main** pushes (`bbd29a7` 14:49:58 + `10c409d` 14:52:10 Production deployments; GitHub "Production" target_urls are preview aliases — the prod alias is truth). **Live confirm**: served entry `assets/index-BTWK-BAA.js`; org chunk `org-26G3JjL3.js` contains **`razorpay-plan-link` (1) and zero `cashfree-plan-link`** — the Razorpay money path is LIVE on https://sitetrackpro.in.
- **Next (user/founder)**: real transaction — org billing → Buy → Razorpay hosted link → pay (UPI/card) → webhook activates plan; watch for the admin activation email; optional residual cleanup: decide fate of `cashfree-plan-link` EF file (still in repo, satisfies smoke required-file entry; `isCashfreeConfigured`/`buildSubscriptionRequest` in `src/lib/integrations/cashfree.ts` remain for the Cashfree subscription rail).