...
`#loginOtp` input in `shell feature index (removed)`. The promoter /
...
- `docs/setup/SIGNUP_EMAIL_RATELIMIT_RUNBOOK.md` — this doc
...
- Existing OTP fallback code:
  ...
  `shell feature index (removed)` `submitOtp` handler
