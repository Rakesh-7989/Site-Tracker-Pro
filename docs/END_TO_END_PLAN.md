...
| **P-F — General issues sweep** | Remaining errors/better-ideas from deep-dives. | F1 sweep → F2 fixes → F3 final re-check → testing loop → release | ✅ **F1+first F2 fix shipped** (2026-08-15, commit `72d66e3`, migration **196** `signup_pay_amount` applied live + Deploy/CI green + live 200): F1 sweep found the `/pay` page derived its amount from the FRONTEND `PLAN_TIERS` (basic/pro/business only) — a `custom`-plan signup rendered a **₹0 / amount-less UPI QR** (DB `plans` has `custom` ₹79,990/yr), and any recorded charge (`paid_amount_paise`, mig 195) / DB price was ignored, so the QR amount could drift from the Cashfree link amount. **F2 fix**: `get_signup_for_pay` recreated (DROP+CREATE, anon/authenticated re-granted) to also return `plan_amount_inr` (plans.yearly_inr × 1.18 GST — same math as cashfree-checkout) + `paid_amount_paise`; `paymentQueries.ts` `SignupForPay` extended + pure `resolveSignupAmount` (recorded paise → DB plan amount → legacy PLAN_TIERS → null); `PayView` uses the resolver and renders a **contact-us state** instead of a ₹0 QR when nothing resolves; `tests/app/paymentQueries.test.ts` 11; smoke marker `resolveSignupAmount` (388 checks). Live anon probe: custom request → `plan_amount_inr=94388` (₹79,990×1.18). Remaining F2/F3 candidates from the sweep (foundation issues only, no product gaps): apply migration 196 — **done**; nothing else tracked yet. |
