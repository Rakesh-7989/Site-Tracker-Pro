...
| **P-F — General issues sweep** | Remaining errors/better-ideas from deep-dives. | F1 sweep → F2 fixes → F3 final re-check → testing loop → release | ✅ **F1+F2 shipped** (2026-08-15, commits `72d66e3` + `233bdf1`, migration **196** applied live + Deploy/CI green + live 200): **F1** built a **column-drift scanner** (parses every frontend `.from().select()` incl. embed aliases against live `information_schema`) → **F2 fixes** (5 real PGRST204 bugs that silently broke live views): ① `monthlyStatementQueries` `po_receipts.project_id` doesn't exist — project is via `po_id→purchase_orders`; now selects `po:purchase_orders(project_id)` + `.in("po.project_id", ids)` + mapper reads `po?.project_id` (embed-filter syntax verified live, 200); ② `SiteWallKioskView` queried nonexistent `updates` table + `safety.title` — now `site_updates`/`notes` + `safety.description`; ③ `DailySnapshotView` `updates` count → `site_updates`; ④ `HandoverPacketView` selected `projects.slug/started_at/completed_at/address` (none exist) — now `location→address`, `start_date→started_at`, `expected_end_date→completed_at`. Earlier `72d66e3`: `get_signup_for_pay` (mig 196) returns `plan_amount_inr` + `paid_amount_paise`; `resolveSignupAmount` (recorded paise → DB plan → PLAN_TIERS → null); PayView contact-us state instead of ₹0 QR; `tests/app/paymentQueries.test.ts` 11. Gate: tsc · lint · build · smoke **388** · vitest **199/2359** · e2e-mock 11/11. F3 re-check: scanner re-run clean. |
