...
| **P-E — Payment-at-signup + temp password + forced change** | Signup payment → email temp password → sign in → mandatory new password. | E1 fix cashfree-webhook dead end (never marks signup paid) · E2 billing_history wiring · E3 temp-password email + admin-API user · E4 force password change · E5 tests | ✅ **complete** (2026-08-15, commit `f8998b8`, migration **195** `signup_payment_password` applied live + EFs deployed + Deploy/CI green + live 200): **E1** `cashfree-webhook` now handles Cashfree **PAYMENT_LINK_EVENTs** (the one-time signup payment from cashfree-checkout) — reconciles `signup_requests` via `link_meta.signup_request_id` or the stashed `payment_ref`, marks `payment_status='paid'` + `paid_at` + `paid_amount_paise` (from gateway order paise), ack 200 for orphan/idempotent/non-PAID, fire-and-forget Resend confirmation; subscription path unchanged. **E2** `review_signup_request` seeds `billing_history` (provider `cashfree`, `external_id=payment_ref`, `amount=paid_amount_paise`, `status='succeeded'`, `paid_at`) once the approved org exists (non-fatal). **E3** provisions NEW users via `admin.auth.admin.createUser` + generated temp password (Resend email) + `profiles.must_change_password=true`; existing emails keep the magic-link fallback. **Migration 195** adds `signup_requests.paid_amount_paise`, `profiles.must_change_password`, self-service `clear_my_must_change_password()` RPC (applied + pg-verified). **E4** frontend forced change: `AuthUser.mustChangePassword` + `fetchAuthSession` mapping + `postLoginPathForSession` guard (single choke point — org + staff lanes + signed-in `<Navigate>`) → new `/auth/change-password` view (updatePassword + RPC + land). **E5** EF source-contract tests (`tests/efPaymentPhase.test.ts` 10) + loginRouting gate tests (+3). Gate: tsc ✓ lint ✓ vitest 198/2348 ✓ smoke 387 ✓ build ✓ e2e-mock 11/11 ✓. **⚠️ founder action: set `CASHFREE_WEBHOOK_SECRET` in Supabase EF secrets** (matches the Cashfree dashboard webhook secret, `docs/CASHFREE_ONBOARDING.md`) — webhook 500s without it. |
| **P-F — General issues sweep** | Remaining errors/better-ideas from deep-dives. | F1 sweep → F2 fixes → F3 final re-check → testing loop → release | ✅ **P-F shipped** (2026-08-15, commits `72d66e3` + `233bdf1` + `760c37b`, migration **196** applied live + Deploy/CI green + live 200): **F1** built a **column-drift scanner** (`scripts/check-column-drift.mjs` — parses every frontend `.from().select()` incl. embed aliases against live `information_schema`, resolves FK embeds + const-ident selects + count-form selects, flags ambiguous multi-FK embeds) → **F2 fixes** (5 real PGRST204 bugs that silently broke live views): ① `monthlyStatementQueries` `po_receipts.project_id` doesn't exist — now `po:purchase_orders(project_id)` + `.in("po.project_id", ids)`; ② `SiteWallKioskView` nonexistent `updates` table + `safety.title` → `site_updates`/`notes` + `safety.description`; ③ `DailySnapshotView` `updates` count → `site_updates`; ④ `HandoverPacketView` `projects.slug/started_at/completed_at/address` → `location→address` etc. Earlier `72d66e3`: `get_signup_for_pay` (mig 196) `plan_amount_inr`/`paid_amount_paise`. **F3 — permanent CI gate shipped** (`760c37b`): scanner promoted to a reusable gate — `npm run check:columns` + a CI step in `.github/workflows/ci.yml` that self-SKIPs (exit 0) when `SUPABASE_DB_URL` isn't set, so a missing secret never blocks; **hardened scanner + 5 more real drifts it found and fixed**: ⑤ `LabourKioskView`/`DailySnapshotView` queried a phantom `labour` table (404 PGRST205) — rewritten to the real `labour_register` + `attendance` model (kiosk clock-in/out now inserts/updates `attendance` rows, `attendee_kind='labour'` with `labour_id`, today-only list); ⑥ `deliverableQueries` `req_profiles(name)`/`closed_profiles(name)` bare aliases (400 PGRST200) → `req_profiles:requested_by(name)`/`closed_profiles:closed_by(name)`; ⑦ `timeQueries` `profiles(name)` ambiguous on time_entries (300 PGRST201, two FKs to profiles) → `profile:profile_id(name)`; ⑧ `platformUsageQueries` `org_members.id` doesn't exist → `profile_id` count. Scanner parser fixes: const-ident `.select(ROW_SELECT)` resolution, count-form `.select("id", {count})`, embed-by-target-name + ambiguity detection. Gate: tsc · lint 0 err · build · smoke **388** · vitest **199/2359** · e2e-mock 11/11 · CI (3 jobs) + Vercel Deploy green · live 200. **`SUPABASE_DB_URL` GitHub secret set** (2026-08-15) — the CI gate now runs against the live DB on every push to main/prod (previously it printed SKIP until the secret landed). |
...
- **P-F follow-up shipped** (2026-08-15, commit `6d9f83e`, migration **197** `kiosk_clock_out` applied live + Deploy/CI green + live 200): the F3-rewritten Labour kiosk's clock-out did a raw `attendance` UPDATE, but `attendance_update` RLS is **PM+ only** → a contractor/site_inspector kiosk operator (who CAN clock in — `attendance_insert` is any project member) hit 42501 on clock-out. Fix: **SECURITY DEFINER RPC `kiosk_clock_out(p_attendance_id, p_out_time, p_hours)`** gated by the same membership check (`project_id ∈ user_project_ids()`), writes only `out_time`+`hours`, refuses not-checked-in rows (double clock-out → 42501). Direct UPDATE/DELETE stays PM+ (correcting kiosk errors). Frontend `LabourKioskView` now calls the new `clockOutAttendance` helper in `attendanceQueries.ts` (RPC-backed, testable). Verified live: anon 42501 · member clock-out OK · double clock-out guarded · grants authenticated-only. Gate: tsc ✓ lint 0 err ✓ vitest **200/2367** ✓ smoke 388 ✓ build ✓ e2e-mock 11/11 ✓.
