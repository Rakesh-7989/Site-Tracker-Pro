...
The DevOps Agent drafted a CI workflow but the push token for this branch lacks the GitHub `workflow` OAuth scope, so the file ships in `docs/CI_WORKFLOW.yml` instead of `.github/workflows/ci.yml`. Apply it manually in one of two ways.
...
git mv docs/CI_WORKFLOW.yml .github/workflows/ci.yml
...
3. Paste the contents of `docs/CI_WORKFLOW.yml`.
